...
class pw_new_user_approve {
	...
	public function default_authentication_message( $status ) {
		...
		if ( $status == 'pending' ) {
			$message = __( '<strong>ERROR</strong>: Su cuenta todavía está pendiente de aprobación.', 'new-user-approve' );
			$message = apply_filters( 'new_user_approve_pending_error', $message );
		} else if ( $status == 'denied' ) {
			$message = __( '<strong>ERROR</strong>: Su cuenta ha sido negada al acceso a este sitio.', 'new-user-approve' );
